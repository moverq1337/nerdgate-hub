...
main() {
  ...
  say ""
  say "This will stop NerdGate Hub and Traefik for this install."
  ...
  say ""
  if confirm "Remove install directory and all data"; then
    case "$install_dir" in
      "/"|"/opt"|"$HOME")
        die "refusing to remove unsafe directory: $install_dir"
        ;;
    esac
    rm -rf "$install_dir"
